...
const clienteCollection = collection(db, "Pergunta");

export default function PerguntaForm({ route }) {
    const Navigator = useNavigation();
    const [carregando, setCarregando] = useState(false)
    const [erro, setErro] = useState(false)
    const [pergunta, setpergunta] = useState({ titulo: "", quiz: route.params })
    const [quizId, setQuizId] = useState(route.params);
    ...
    async function cadastrar() {
        setCarregando(true);
        try {
            setpergunta({ ...pergunta, quiz: quizId });
            console.log(pergunta);
            const docRef = await addDoc(clienteCollection, pergunta);
                ...
                {carregando ? (
                    ...
                    <>
                        ...
                        <ScrollView>
                            ...
                            <TextInput
                                style={styles.input}
                                onChangeText={(text) => setpergunta({ titulo: text, quiz: quizId })}
