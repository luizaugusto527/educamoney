...
const clienteCollection = collection(db, "Puzzle");

export default function QuestaoDetalhe({ route }) {

    const Navigator = useNavigation();
    const [carregando, setCarregando] = useState(false)
    const [erro, setErro] = useState(false)
    const [quiz, setQuiz] = useState(route.params)
    ...
    const toggleVisibilityError = (excluida) => {
        if (excluida)
        setOperacao("Excluir");
        excluida ? Navigator.navigate("Puzzle") : null
    ...
    async function atualizar() {
        setCarregando(true);
        try {
            const docRef = doc(db, 'Puzzle', quiz.id);
    ...
    async function excluir() {
        ...
        try {
            ...
            console.error("Erro ao atualizar a quiz:", error);
            setCarregando(false);
        ...
        <View style={styles.container}>
            ...
            <View style={styles.form}>
                {carregando ? (
                    ...
                    <>
                        <Text style={styles.o}>Quiz</Text>
                        {erro && <Text style={[styles.erroLogin]}>{erro}</Text>}
                        <ScrollView>
                            <Text style={styles.label}>Titulo</Text>
                            <TextInput style={styles.input} onChangeText={(text) => setQuiz({ ...quiz, titulo: text })} value={quiz.titulo} />
                           ...
                           <View style={styles.box}>
                                ...
                                <Text style={styles.textBox}>Quiz {operacao} com sucesso!!!</Text>
